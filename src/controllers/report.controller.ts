...
export default class ReportController extends BaseController {
    ...
    protected async notRegistered(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        try {
            ...
            principal_name,
            principal_mobile,
            principal_email FROM organizations WHERE status='ACTIVE' && district LIKE ${districtFilter} && NOT EXISTS(SELECT mentors.organization_code  from mentors WHERE organizations.organization_code = mentors.organization_code) `, { type: QueryTypes.SELECT });
            if (!mentorsResult) {
                throw notFound(speeches.DATA_NOT_FOUND)
            }
            if (mentorsResult instanceof Error) {
    ...
    protected async mentorsummary(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        try {
            ...
            org.organization_count - (org.total_registered_teachers) AS total_not_registered_teachers
            FROM (
                SELECT 
                    o.district,
        ...
        UNION ALL SELECT 
            ...
            SUM(organization_count - (total_registered_teachers))
        FROM
            (SELECT 
                o.district,
                    COUNT(DISTINCT o.organization_id) AS organization_count,
        ...
        WHERE
            og.status = 'ACTIVE'
                && og.district like ${districtFilter}
                 order by district,mn.full_name,t.team_name,st.full_name;`, { type: QueryTypes.SELECT });
            data=summary;
            if (!data) {
                throw notFound(speeches.DATA_NOT_FOUND)
            }
    ...
    protected async getmentorDetailsreport(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
        ...
        WHERE
            og.status = 'ACTIVE'
                && og.district like ${districtFilter} order by district;`, { type: QueryTypes.SELECT });
            data=summary;
            if (!data) {
                throw notFound(speeches.DATA_NOT_FOUND)
            }
